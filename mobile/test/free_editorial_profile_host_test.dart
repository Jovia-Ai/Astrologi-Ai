...
Map<String, dynamic> _cardMap(int ord, String domain, String planet, String type, String key, String title) =>
    <String, dynamic>{
...
Map<String, dynamic> _payloadA() => <String, dynamic>{
      ...
      'editorial_profile': {
        ...
        'cards': <Map<String, dynamic>>[
          ...
          _cardMap(1, 'identity', 'sun', 'planet_house', 'sun_house_4', 'Güneş 4. Ev'),
          _cardMap(2, 'emotion', 'moon', 'planet_sign', 'moon_gemini', 'Ay İkizler'),
        ],
...
Map<String, dynamic> _payloadB() => <String, dynamic>{
      'editorial_profile': {
        ...
        'locale': 'tr-TR',
        'cards': <Map<String, dynamic>>[
          _cardMap(0, 'identity', 'sun', 'planet_sign', 'sun_capricorn', 'Güneş Oğlak'),
        ],
        'diagnostics': {'missing_keys': <dynamic>[], 'fallback_used': false},
      },
    };
...
void main() {
  test('1. flag off preserves existing path (no narrow)', () {
    final d = decideFreeEditorialHost(flagEnabled: false, payload: _payloadA());
    expect(d.showNarrow, false);
    expect(d.reason.code, 'EDITORIAL_FLAG_MISMATCH');
  });

  test('2. flag on + valid payload opens narrow', () {
    final d = decideFreeEditorialHost(flagEnabled: true, payload: _payloadA());
    expect(d.showNarrow, true);
    expect(d.reason.code, 'EDITORIAL_SHOW');
    expect(d.model.cards.length, 3);
  });

  test('3. flag on + absent payload never displays mock', () {
    final d = decideFreeEditorialHost(flagEnabled: true, payload: <String, dynamic>{'core_story': 'x'});
    expect(d.showNarrow, false);
    expect(d.reason.code, 'EDITORIAL_PAYLOAD_ABSENT');
    expect(d.model.cards, isEmpty);
  });

  test('4. narrow and legacy never render together', () {
    // decision is exclusive: showNarrow implies a valid model; otherwise host keeps legacy.
    final on = decideFreeEditorialHost(flagEnabled: true, payload: _payloadA());
    expect(on.showNarrow && on.model.isValid, true);
    final off = decideFreeEditorialHost(flagEnabled: false, payload: _payloadA());
    expect(off.showNarrow, false);
  ...
  test('5. card order from backend remains unchanged', () {
    final d = decideFreeEditorialHost(flagEnabled: true, payload: _payloadA());
    expect(d.model.cards.map((c) => c.primaryKey).toList(),
        ['sun_leo', 'sun_house_4', 'moon_gemini']);
  });

  test('6. user/profile change replaces the card set', () {
    final a = decideFreeEditorialHost(flagEnabled: true, payload: _payloadA());
    final b = decideFreeEditorialHost(flagEnabled: true, payload: _payloadB());
    expect(a.model.cards.map((c) => c.primaryKey).toList(),
        isNot(equals(b.model.cards.map((c) => c.primaryKey).toList())));
    expect(b.model.cards.single.primaryKey, 'sun_capricorn');
  });

  test('7. stale cards from previous profile are not retained', () {
    // payload nulled on profile change/error -> no prior cards survive
    final cleared = decideFreeEditorialHost(flagEnabled: true, payload: null);
    expect(cleared.showNarrow, false);
    expect(cleared.model.cards, isEmpty);
    expect(cleared.reason.code, 'EDITORIAL_PAYLOAD_ABSENT');
  });

  test('8. malformed payload produces the safe state', () {
    final bad1 = decideFreeEditorialHost(flagEnabled: true, payload: <String, dynamic>{'editorial_profile': 'oops'});
    expect(bad1.showNarrow, false);
    expect(bad1.reason.code, 'EDITORIAL_PAYLOAD_INVALID');
    final bad2 = decideFreeEditorialHost(flagEnabled: true, payload: <String, dynamic>{'editorial_profile': {'version': 'x'}});
    expect(bad2.reason.code, 'EDITORIAL_PAYLOAD_INVALID');
    final empty = decideFreeEditorialHost(flagEnabled: true, payload: <String, dynamic>{
  ...
  test('9. Koruyucu dalga absent from narrow path', () {
    for (final f in [
  ...
  test('10. Profile V8 / supporting threads not consulted by narrow path', () {
    for (final f in [
      'lib/app/profile/free_editorial_profile_host.dart',
      'lib/app/profile/free_editorial_profile_adapter.dart',
    ]) {
      final src = _codeOf(f);
      for (final forbidden in ['profile_v8', 'sections_v2', 'supporting_thread', 'ProfileV8Adapter']) {
        expect(src.contains(forbidden), false, reason: '$f must not reference $forbidden');
      }
    }
    // decision reads only editorial_profile
    expect(_codeOf('lib/app/profile/free_editorial_profile_host.dart').contains("'editorial_profile'"), true);
  });

  test('11. Tam Okuma is not embedded in narrow path', () {
    for (final f in [
      'lib/app/profile/free_editorial_profile_host.dart',
      'lib/app/profile/free_editorial_profile_view.dart',
      'lib/app/profile/free_editorial_profile_adapter.dart',
