...
// based ONLY on (1) the default-off flag and (2) a valid editorial_profile
// payload. Records a typed reason for diagnostics. No legacy fallback consulted,
// no mutation, no mock content.

import 'free_editorial_profile_adapter.dart';

enum FreeEditorialHostReason {
  show,
  flagDisabled, // EDITORIAL_FLAG_MISMATCH
...
extension FreeEditorialHostReasonCode on FreeEditorialHostReason {
  String get code {
    switch (this) {
      ...
      case FreeEditorialHostReason.flagDisabled:
        return 'EDITORIAL_FLAG_MISMATCH';
...
FreeEditorialHostDecision decideFreeEditorialHost({
  ...
  if (!flagEnabled) {
    return const FreeEditorialHostDecision(
      showNarrow: false,
      reason: FreeEditorialHostReason.flagDisabled,
