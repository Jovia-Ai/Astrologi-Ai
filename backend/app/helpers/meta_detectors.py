...
def normalize_sign(value: str | None) -> str:
    if not value:
        return ""
    return str(value).strip().lower()


def get_element_for_sign(value: str | None) -> str | None:
    normalized = normalize_sign(value)
    if not normalized:
        return None
    title_case = normalized.capitalize()
    element = ELEMENT_MAP.get(title_case)
    return element.lower() if isinstance(element, str) else None
