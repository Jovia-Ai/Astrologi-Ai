...
| 5.2 | Composed detail renderer — hidden/private (Phase-4 work) | `composed_detail_renderer.py` + canonical prose home `backend/app/natal/narrative/phrase_lib_tr_deep_read.py` | active-narrow | **CONSOLIDATE — phased (S4a done, S4b deferred)** | S4 pre-audit (`docs/system/audits/phase4_to_phrase_lib_migration_pre_audit.md`) surfaced a contract gap: `render_block_template` emits ONE block with A/B/C/D stylistic modes; Phase-4 emits FIVE slides with surface_role semantic beats. Direct drop-in into `BODY_TEMPLATES_TR` fights the model. **S4a (commit `9e07ed2`)**: prose-as-data migration — Phase-4 slide PROSE moved to canonical `HIDDEN_PRIVATE_DEEP_READ_SLIDES_TR` constant in sibling `phrase_lib_tr_deep_read.py`; builder reduced to ~12-line thin adapter; output byte-identical (125 tests pass); +1 shape-guard test. **S4b (deferred)**: introduce `render_slide_flow(...)` near the constants + evolve per-role entries into variant pools — delivers the "şablon hissi olmasın" goal via chart-hash-seeded rotation |
...
| **S4a** | ✅ DONE 2026-05-20 (commit `9e07ed2`) — Phase-4 prose migrated to canonical `phrase_lib_tr_deep_read.HIDDEN_PRIVATE_DEEP_READ_SLIDES_TR` constant (sibling to `phrase_lib_tr_profile.py` per pre-audit model-gap finding); builder reduced from ~90-line inline prose to ~12-line thin adapter; +1 shape-guard test | Low-medium | Atomic byte-identical relocation; 125 regression tests pass | Phase-4 prose now lives in canonical narrative library. Builder is a thin adapter. Future TR copy edits land in the library, not buried in a renderer function. No render-machinery change (S4b scope) |
| **S4b** | (deferred, separate later request) — introduce `render_slide_flow(...)` near `HIDDEN_PRIVATE_DEEP_READ_SLIDES_TR`; evolve per-role entries into variant pools (`{role: [{title, body}, ...]}`); rewire Phase-4 adapter to use chart-hash-seeded selection | Medium-high | Real design + variant authoring work | Delivers the user's "şablon hissi olmasın" goal: chart-specific variation via combinatoric rotation. After S4b, matrix §5.2 fully closed and Phase-4 becomes the first consumer of the consolidated frame engine |
