...
| 7.2 | V26 narrative stack (`build_narrative`, `build_core_story_plan`, `build_domain_narrative_v26`, `render_core_story`, `StylePackV26TR`) | `builders/narrative_binding.py`, `builders/narrative_renderer_v26.py`, `style/style_pack_v26_tr.py` (1,994 lines total) | parallel-older-stack | **FREEZE then DELETE** after trace audit confirms zero runtime callers in canonical natal/synastry/transit chains | Large parallel narrative engine; not central to current public flow; trace before deletion to be safe |
...
| **S2** | Trace audit on V26 stack (7.2) — identify every callsite | Low | Trace only, no code change | Decision data: DELETE-safe or FREEZE-only |
