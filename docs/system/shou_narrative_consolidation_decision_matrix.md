...
| 4.13a | `primitive_engine_v2` + `contradiction_engine` + `layer_arbitrator` | various | active-canonical-upstream | **RESCUE** v2 | Converged path |
| 4.13b | `primitive_engine` (v1) | `primitive_engine.py` | unclear | **FREEZE → DELETE** after trace audit confirms v2 fully replaces it | Don't keep two primitive engines |
...
| 7.2a | **V26 LIVE branch** — `build_core_story_plan` + `render_core_story` | `builders/narrative_binding.py`, `builders/narrative_renderer_v26.py` (symbol-level subset) | active-canonical | **CONSOLIDATE-with-contract-preservation** (S2.1 audit decided) | S2.1 audit (`docs/system/audits/v26_core_story_downstream_consumer_audit.md`) found three concrete downstream consumers: (a) `PublicNatalView.core_story` public field, mobile-consumed via `chart_lab_page.dart`; (b) `profile_v8.identity_axis_body` fallback in `profile_v8_payload_builder.py:1749`; (c) `core_story_ui` builder + `data_quality` payload, both fed by `core_story_plan`. Migration must preserve public field shape (mobile contract unchanged). Prerequisite: snapshot-test coverage on `public["core_story"]` content. New home: Signature renderer (4.4) or a dedicated `core_story_module.py`. Sequencing in S2.1 §11 |
...
| **S2.1.1** | Add baseline snapshot tests on `public["core_story"]` content + shape for canonical natal fixtures (e.g. 1996, 2007, others) before any V26 LIVE refactor begins | Low | Test additions only, no production change | Closes Risk 1 from S2.1 audit; protects mobile + profile_v8 fallback contract during eventual migration |
| **S2.1.2** | Design the V26 LIVE migration target: new module location (`signature_renderer` 4.4 vs new dedicated `core_story_module.py`) + migration sequencing. Output: design doc, not code | Low | Design only | Concrete §13.2-ready bounded request for the actual refactor |
| **S2.1.3** | Execute the migration: extract `render_core_story` + `build_core_story_plan` into the chosen target; preserve all output shapes byte-identical; flag-gated; tests green | Medium | Coverage from S2.1.1 + flag gating mitigates | Consolidates V26 LIVE into canonical stack; V26 LIVE files become deletable |
| **S2.2** | DELETE the V26 dead branch (7.2b): remove `build_narrative`, `build_domain_narrative_v26`, `StylePackV26TR`, dead import in natal route; delete `style_pack_v26_tr.py`. Verification: run full backend test suite — zero failures required | Low-medium | The 'dead' label rests on grep; the test-suite verification is the second proof. Risk: an indirect dynamic import / getattr lookup the grep missed. Mitigation: stage as remove-then-restore-on-failure | ~135 lines of dead code removed from `narrative_binding.py` + `narrative_renderer_v26.py`, plus full 105-line `style_pack_v26_tr.py`. Smaller repo, less cognitive load on readers |
| **S3** | Trace audit on `primitive_engine` v1 vs v2 (4.13) | Low | Trace only | Decision data: DELETE-safe or RESCUE-both |
