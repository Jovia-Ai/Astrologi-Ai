...
| 7.2a | **V26 LIVE branch** — `build_core_story_plan` + `render_core_story` | `builders/narrative_binding.py`, `builders/narrative_renderer_v26.py` (symbol-level subset) | active-canonical (S2 audit reveal) | **MIGRATE-callers-first** — NOT delete-safe | S2 trace audit (`docs/system/audits/v26_trace_audit.md`) found these symbols are on the canonical natal `/interpret`, `/interpret/ui`, `/interpret/premium`, `/interpret/premium/ui` runtime path. They write `response["core_story_plan"]` and `response["core_story"]` which `build_public_natal_view` consumes. Cannot delete; must migrate. **Awaiting micro-audit on downstream consumers of those response fields** (S2.1) to decide RESCUE-as-is vs CONSOLIDATE into Signature renderer (4.4) |
...
| **S2.1** | Micro-audit on downstream consumers of `response["core_story"]` + `response["core_story_plan"]` after `build_public_natal_view` — who reads them? mobile? tests? other backend code? | Low | Trace only | Decision data for 7.2a: RESCUE-as-is (if fields are stably consumed downstream and migration would break consumers) vs CONSOLIDATE into Signature renderer (4.4) (if fields are vestigial or can be replicated) |
